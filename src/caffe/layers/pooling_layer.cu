...
template <typename Dtype>
__global__ void SamplePoolForward(const int nthreads,
    const Dtype* const bottom_data, const int num, const int channels,
    const int height, const int width, const int pooled_height,
    const int pooled_width, const int kernel_h, const int kernel_w,
    const int stride_h, const int stride_w, const int pad_h, const int pad_w,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    int sampleidx = hstart * width + wstart;
    Dtype sampleval = bottom_slice[sampleidx];
    top_data[index] = sampleval;
    if (mask) {
      mask[index] = sampleidx; 
    } else {
      top_mask[index] = sampleidx; 
    }
  }
}
...
void PoolingLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  case PoolingParameter_PoolMethod_MAX:
    ...
    if (this->layer_param_.pooling_param().sample()){
	SamplePoolForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
