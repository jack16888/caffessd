...
void DataTransformer<Dtype>::Transform(const Datum& datum,
  ...
  *do_mirror = param_.mirror() && Rand(2);
  ...
  const bool quant_enable = param_.quant_enable();
  ...
  for (int c = 0; c < datum_channels; ++c) {
    for (int h = 0; h < height; ++h) {
      for (int w = 0; w < width; ++w) {
        ...
        //GTI insert: quant vals from [0, 1>>bw_in] to [0, 1>>bw_out]
        if(quant_enable) {
            //perform shift
            int bw_in = param_.bw_in();
            int bw_out = param_.bw_out();
            uint16_t temp = static_cast<Dtype>(static_cast<uint16_t>(datum_element));
            temp = (bw_in > bw_out) ? ((temp >> (bw_in - bw_out - 1)) + 1) >> 1 : temp;
            uint16_t max_val = (1 << bw_out) - 1;
            //clipping
            if(temp < 0) {
                temp = 0;
            }
            ...
            transformed_data[top_index] = temp;
        } else {
            if (has_mean_file) {
    ...
    for (int w = 0; w < width; ++w) {
      ...
      for (int c = 0; c < img_channels; ++c) {
        ...
        if(quant_enable) {
            ...
            int bw_in = param_.bw_in();
            int bw_out = param_.bw_out();
            uint16_t temp = static_cast<Dtype>(static_cast<uint16_t>(pixel));
            temp = (bw_in > bw_out) ? ((temp >> (bw_in - bw_out - 1)) + 1) >> 1 : temp;
            uint16_t max_val = (1 << bw_out) - 1;
            //clipping
            if(temp < 0) {
    ...
    for (int w = 0; w < width; ++w) {
      for (int c = 0; c < channels; ++c) {
        ...
        if(quant_enable) {
            ...
            int bw_in = param_.bw_in();
            int bw_out = param_.bw_out();
            uint16_t temp = static_cast<Dtype>(static_cast<uint16_t>(data[idx]));
            temp = (bw_in > bw_out) ? ((temp >> (bw_in - bw_out - 1)) + 1) >> 1 : temp;
            uint16_t max_val = (1 << bw_out) - 1;
            //clipping
            if(temp < 0) {
            ...
            ptr[img_idx++] = static_cast<uchar>(temp);
        } else {
            if (has_mean_file) {                                                  
        ...
        for (int h = 0; h < height; ++h) {
          ...
          if (do_mirror) {
            ...
            for (int w = 0; w < width; ++w) {
              ...
              int bw_in = param_.bw_in();
              int bw_out = param_.bw_out();
              uint16_t temp = static_cast<Dtype>(static_cast<uint16_t>(input_data[data_index_h + w]));
              temp = (bw_in > bw_out) ? ((temp >> (bw_in - bw_out - 1)) + 1) >> 1 : temp;
              uint16_t max_val = (1 << bw_out) - 1;
              //clipping
              if(temp < 0) {
              ...
              transformed_data[top_index_w-w] = temp;
            }
          } else {
            for (int w = 0; w < width; ++w) {
              //perform shift                                                             
              ...
              transformed_data[top_index_h + w] = temp; 
            }
          }
        }
      }
